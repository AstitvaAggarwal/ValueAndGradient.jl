...
function ValueAndGradient.value_and_pullback!!(
        f::F, ȳ, backend::AutoEnzyme, x::AbstractArray;
        ad_cache=nothing, canonical_tangents=false, kwargs...) where {F}
    ad_cache !== nothing && @warn "AutoEnzyme does not support ad_cache; it will be ignored."
    y = f(x)
    ∂x = zero(x)
...
function ValueAndGradient.value_and_pushforward!!(
        f::F, ẋ::AbstractArray, backend::AutoEnzyme, x::AbstractArray;
        ad_cache=nothing, canonical_tangents=false, kwargs...) where {F}
    ad_cache !== nothing && @warn "AutoEnzyme does not support ad_cache; it will be ignored."
    y = f(x)
    result = Enzyme.autodiff(Enzyme.Forward, f, Enzyme.Duplicated,
